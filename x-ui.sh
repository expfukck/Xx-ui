...
install() {
    bash <(curl -Ls https://raw.githubusercontents.com/expfukck/Xx-ui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    if [[ $yn =~ "Y"|"y" ]]; then
        ...
        last_version=$(curl -Ls "https://api.github.com/repos/expfukck/Xx-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ -z "$last_version" ]]; then
            red "检测 x-ui 版本失败，可能是超出 Github API 限制，正在使用备用源检测最新版本"
            last_version=$(curl -sm8 https://raw.githubusercontents.com/expfukck/Xx-ui/main/config/version)
            if [[ -z "$last_version" ]]; then
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(archAffix).tar.gz https://github.com/expfukck/Xx-ui/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        wget -N --no-check-certificate https://raw.githubusercontents.com/expfukck/Xx-ui/main/x-ui.sh -O /usr/bin/x-ui
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/expfukck/Xx-ui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
————————————————
 ...
 ${GREEN}17.${PLAIN} 安装并配置CloudFlare WARP
    ...
    case "${num}" in
        ...
        14) install_bbr ;;
        15) wget -N https://raw.githubusercontents.com/expfukck/acme-1key/master/acme1key.sh && bash acme1key.sh && before_show_menu ;;
        16) open_ports ;;
        17) wget -N --no-check-certificate https://raw.githubusercontents.com/expfukck/Misaka-WARP-Script/master/misakawarp.sh && bash misakawarp.sh && before_show_menu ;;
        *) red "请输入正确的数字 [0-17]" ;;
    esac
}
